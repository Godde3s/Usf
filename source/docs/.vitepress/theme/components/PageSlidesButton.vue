...
// Locale-aware labels for the slides feature
const SLIDES_LABELS = {
  en: { btn: 'Slides', open: 'Open slides', close: 'Close slides', playing: 'Page slides', generating: 'Generating slides...', cont: 'cont.' },
  es: { btn: 'Diapositivas', open: 'Abrir diapositivas', close: 'Cerrar diapositivas', playing: 'Diapositivas de página', generating: 'Generando diapositivas...', cont: 'cont.' }
}
const slidesLabels = () => {
  const path = typeof window !== 'undefined' ? window.location.pathname : ''
  const m = path.match(/\/(en|es)\//)
  return SLIDES_LABELS[m ? m[1] : 'en'] || SLIDES_LABELS.en
}
...
const createContinuationHeading = (heading) => {
  ...
  label.textContent = slidesLabels().cont
...
<template>
  <button
    ...
    type="button"
    :aria-label="slidesLabels().open"
    :title="slidesLabels().btn"
    @click="openSlides"
  >
    <el-icon :size="16">
      <Present />
    </el-icon>
    <span class="ev-slides-button-label">{{ slidesLabels().btn }}</span>
  </button>
  ...
  <Teleport to="body">
    <div
      ...
      role="dialog"
      aria-modal="true"
      :aria-label="slidesLabels().playing"
      @keydown.esc.stop.prevent="closeSlides"
    >
      <button
        class="ev-slides-close"
        type="button"
        :aria-label="slidesLabels().close"
        :title="slidesLabels().close"
        @click="closeSlides"
      >
        <el-icon :size="18">
          <Close />
      ...
      <div class="ev-slides-shell">
        ...
        >
          ...
          <span>{{ slidesLabels().generating }}</span>
        </div>
