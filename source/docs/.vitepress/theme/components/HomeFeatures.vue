...
const normalizeLocaleCode = (value) => {
  const code = value ? value.toLowerCase() : 'en'
...
const topPromoCopy = {
  en: {
    text: 'Need this kind of engineering? Browse the projects and hire me.',
    cta: 'See the work ›'
  },
  es: {
    text: '¿Necesitas este tipo de ingeniería? Explora los proyectos y contrátame.',
    cta: 'Ver el trabajo ›'
  }
}
...
const topPromo = computed(() => {
  const code = normalizeLocaleCode(lang.value)
  const copy = topPromoCopy[code] || topPromoCopy.en
  return {
    text: copy.text,
    cta: copy.cta,
    link: `/${code}/projects/netpilot/`
  }
})
...
onMounted(() => {
  ...
  const normalizedPath = currentPath.endsWith('/')
    ? currentPath
    : `${currentPath}/`
  const localeHomeSuffixes = ['/en/', '/es/']
        ...
        <div class="nav-cluster">
          ...
          >
            <img
              ...
              :src="withBase('/assets/usf-logo-hd.svg')"
              :alt="t.nav.title"
              width="64"
              height="30"
              decoding="async"
