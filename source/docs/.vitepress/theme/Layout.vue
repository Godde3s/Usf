...
// Locale-aware UI labels for reading settings & sidebar controls
const UI_LABELS = {
  en: { fontSize: 'Font size', def: 'Default', lineHeight: 'Line height', tighter: 'Tighter', looser: 'Looser', reading: 'Reading settings', expand: 'Expand sidebar', collapse: 'Collapse sidebar' },
  es: { fontSize: 'Tamaño de fuente', def: 'Predeterminado', lineHeight: 'Interlineado', tighter: 'Más estrecho', looser: 'Más amplio', reading: 'Ajustes de lectura', expand: 'Expandir índice', collapse: 'Contraer índice' }
}
const uiLabels = () => {
  const path = typeof window !== 'undefined' ? window.location.pathname : ''
  const m = path.match(/\/(en|es)\//)
  return UI_LABELS[m ? m[1] : 'en'] || UI_LABELS.en
}
  ...
  <DefaultTheme.Layout>
    <template v-if="!isHomePage && !isWelcomePage" #nav-bar-title-before>
      <button
        ...
        type="button"
        :aria-label="sidebarCollapsed ? uiLabels().expand : uiLabels().collapse"
